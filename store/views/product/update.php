...
$this->title = 'Изменить: ' . $model->translate->name;
?>
                    ...
                    <div class="page-header"><?= Html::encode($this->title) ?>
                        ...
                        <?php if(($model->status == 1 || $model->status == 2) && $model->category->on_main != 1) { ?>
                            <div class="pull-right">
                                <a href="<?=Yii::$app->params['domain']?>/product/<?=$model->url?>" target="_blank" data-pjax="0">Страница объявления <i class="fa fa-share"></i></a>
                            </div>
                        <?php } ?>
                    </div>
                    <p></p>
